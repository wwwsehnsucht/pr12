...
app.delete("/tasks", async (req, res) => {
  try {
    ...
    res.json({ message: "every tasks is deleted" });
  } catch (error) {
    console.error("error while cleaning the tasks", error);
    res.status(500).json({ error: "server error while cleaning the tasks" });
  }
});
